// Complete Data Generator - Fills missing months and unifies storage
...
(async function generateCompleteData() {
    ...
    console.log('👤 Generating complete data for user:', currentUser.email);
    
    // South Indian Food Database (same as before)
    ...
    // Step 1: Clear legacy localStorage data
    console.log('🧹 Clearing legacy localStorage data...');
    localStorage.removeItem('nutritionHistory');
    console.log('✅ Legacy data cleared');

    // Step 2: Generate complete 6-month data (August to November)
    const today = new Date();
    const startDate = new Date(today.getFullYear(), 7, 1); // August 1st (month 7 = August)
    
    let currentDate = new Date(startDate);
    let dayCount = 0;
    let totalCalories = 0;

    console.log(`📅 Generating data from ${startDate.toLocaleDateString()} to ${today.toLocaleDateString()}`);
    ...
    console.log(`📊 Generated ${dayCount} days of unified data`);
    console.log(`📊 Average calories per day: ${Math.round(totalCalories/dayCount)}`);
    console.log(`🗓️ Coverage: August, September, October, November 2025`);
    console.log(`💾 All data stored in Firestore (unified storage)`);
    console.log(`🧹 Legacy localStorage data removed`);
    console.log('🔄 Refresh the page to see complete data in History and Analytics!');
})();